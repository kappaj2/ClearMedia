...
public class SourceFileHandler {
	...
	public List<File> getFilePlayList() {
		...
		System.out.println("Value for sourceFolder is  >"+sourceFolder+"<");
		System.out.println("Value for archiveFolder is >"+archiveFolder+"<");
		System.out.println("Value for uploadFolder is  >"+uploadFolder+"<");
		
		stableTime = Long.parseLong(propHandler.getProperty("stable_time"));
		...
		for (Path path : currentContent) {
			fileCurrentContent.add(path.toFile());
		}
		return fileCurrentContent;
	}
		...
		try (DirectoryStream<Path> dirStream = Files.newDirectoryStream(dir,
			...
			for (Path path : dirStream) {
				...
				if (isReqularReadableFile) {
					...
					System.out.println("Found new file - checking age...");
